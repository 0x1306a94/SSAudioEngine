...
@implementation SSMacAudioDevice


@end

@implementation SSMacAudioSession
+ (instancetype)sharedInstance
{
    static SSMacAudioSession * audioSession = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        audioSession = [[SSMacAudioSession alloc] init];
    });
    return audioSession;
}
...
- (NSArray<SSMacAudioDevice *> *)audioDevicesForPropertySelector:(AudioObjectPropertySelector)selector
{
    ...
    for (UInt32 i = 0; i < count; i++)
    {
        ...
        SSMacAudioDevice * device = [[SSMacAudioDevice alloc] init];
